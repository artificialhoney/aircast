...
module.exports = function() {
  ...
  //create AirCast
  var aircast = new AirCast(logger, discovery, proxy, {
    level: program.level
  });
  ...
  apiRouter.put('/devices/:name/connect', function(req, res) {
    if (!aircast.hasDevice()) return res.status(404).send('Device not found');
    device = aircast.setConnect(req.params.name, req.body);
    res.json(device);
  });
  apiRouter.put('/devices/:name/volume', function(req, res) {
    if (!aircast.hasDevice()) return res.status(404).send('Device not found');
    device = aircast.setVolume(req.params.name, req.body);
    res.json(device);
  });
